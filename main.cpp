...
int main() {
    ...
    myAtoms2[6] = new atom(6, 5.0, 3.0, 0.0);
    myAtoms2[7] = new atom(7, 8.5, 5.5, 0.0);
    myAtoms2[8] = new atom(8, 6.5, 7.0, 0.0);
    myAtoms2[9] = new atom(9, 2.4, 11.5, 0.0);
    ...
    myAtoms.emplace_back(atom(6, 5.0, 3.0, 0.0));
    myAtoms.emplace_back(atom(7, 8.5, 5.5, 0.0));
    myAtoms.emplace_back(atom(8, 6.5, 7.0, 0.0));
    myAtoms.emplace_back(atom(9, 2.4, 11.5, 0.0));
    ...
    atom probeAtom = atom(99, 0, 0, 0);
    vector<cell> neighbors;

    for (int m = 0; m < 100; ++m) {

        double xProbe = m*0.1;
        double yProbe = -1.2*(xProbe)+12.0;
        double zProbe = 2.4;



        // probe atom
        probeAtom.setX(xProbe);
        probeAtom.setY(yProbe);
        probeAtom.setZ(zProbe);
        ...
        for (int l = 0; l < atomList.size(); ++l) {
            ...
            if (r <= RCUT) {
                Upot += 4.0 * epsilon * ( pow((sigma/r),12.0) - pow((sigma/r),6.0) );
                nCalcPot += 1;
            }
